...
| **Полный GPU (этот запуск)** | **ViT-B/14** | **448 px** | **16** | **RTX 5070 Ti** |

### 4.2 Гиперпараметры (полный GPU-запуск)

| Параметр | Быстрый тест | Полный GPU |
|---|---|---|
| Backbone | ViT-S/14 | **ViT-B/14** |
| Разрешение | 224 px | **448 px** |
| Патч-векторов K-means | 645 632 (dim=384) | **2 582 528 (dim=768)** |
| Оптимизатор | AdamW | AdamW |
| LR декодера | 1e-4 | 1e-4 |
| LR backbone | — | 1e-5 (с раунда 3) |
| Weight decay | 1e-4 | 1e-4 |
| EMA decay (α) | 0.999 | 0.999 |
| Эпох warm-up | 1 | **5** |
| Раундов × эпох | 2 × 2 | **5 × 10** |
| Порог θ₀ | 0.70 (фикс.) | **0.95 → 0.85** |
| Время обучения | ~10 мин | **2 ч 18 мин** |

### 4.3 Реальные результаты (LoveDA val, RTX 5070 Ti)

![fig6_training_curves](report_figures/fig6_training_curves.png)

**Полный GPU-прогон (ViT-B/14, 448 px, 55 эпох):**

| Этап | Эпохи | Train Loss | Val mIoU (Hungarian) | Уверен. пиксели |
|---|---|---|---|---|
...
**Наблюдения:**
- Loss убывает с 1.60 → 0.15 (−91%) за 55 эпох.
- Лучший результат достигнут **после warm-up** (21.38%) — вдвое лучше быстрого теста (9.86%), преимущественно за счёт ViT-B/14 и разрешения 448 px.
- В раундах 1–2 уверенных пикселей 0% (θ=0.95 слишком высок) — consistency loss не активен, модель постепенно теряет начальное выравнивание.
- С раунда 3 (θ=0.90) consistency loss включается: 12.6% → 59.3% → 82.9% уверенных пикселей. mIoU частично восстанавливается, но не достигает уровня warm-up.
...
- **Деградация mIoU при self-training**: начальный θ₀=0.95 слишком высок — первые 2 раунда работают без consistency loss. Рекомендуется θ₀=0.80–0.85.
...
- Достигает **mIoU 21.38 %** на датасете LoveDA (ViT-B/14, 448 px, RTX 5070 Ti, 2 ч 18 мин);
- Полностью воспроизводим: `make env-gpu && make download && make train-gpu`;
